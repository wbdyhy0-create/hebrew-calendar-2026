...
export async function saveBlobToHandle(handle: SaveHandle, blob: Blob) {
  ...
  // Some File System Access implementations can end up writing 0 bytes when given a Blob directly,
  // especially for large blobs generated after heavy async work. Writing an ArrayBuffer is more reliable.
  if (blob instanceof Blob && blob.size > 0) {
    const buf = await blob.arrayBuffer();
    // Ensure we overwrite from the beginning and drop any placeholder bytes.
    try {
      // Some streams support truncate; ignore if unsupported.
      await (writable as any).truncate?.(0);
    } catch {
      // ignore
    }
    // Prefer the structured write form if available.
    try {
      await (writable as any).write({ type: 'write', position: 0, data: buf });
    } catch {
