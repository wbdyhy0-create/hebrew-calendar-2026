export type DayTextOverride = {
  ...
  imageDataUrl?: string;
  /** When true, do not use global cell image fallback for this day. */
...
export const GLOBAL_CELL_IMAGE_KEY = '__all__';
export const MONTH_PADDING_IMAGE_KEY_PREFIX = '__pad__:';

export function monthPaddingImageKeyFromYearMonth(d: Date): string {
  const y = d.getFullYear();
  const m = String(d.getMonth() + 1).padStart(2, '0');
  return `${MONTH_PADDING_IMAGE_KEY_PREFIX}${y}-${m}`;
}
