...
function setCors(req: VercelRequest, res: VercelResponse) {
  // Allow browser-based publishing from Studio to Display.
  // Security is enforced via `PUBLISH_SECRET` when configured.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // Dynamic import avoids crashing the function at module-load time
    // (which would break CORS preflight with FUNCTION_INVOCATION_FAILED).
    const { kv } = await import('@vercel/kv')

    const cat = sanitizeThemeCatalog(themeCatalog)
    const shouldClearCatalog =
      clearThemeCatalog === true || (Array.isArray(themeCatalog) && (themeCatalog as any[]).length === 0)
    if (shouldClearCatalog) {
      await kv.del(tenantKvKey(tenantId, KV_CATALOG_KEY))
    } else if (cat && cat.length) {
      await kv.set(tenantKvKey(tenantId, KV_CATALOG_KEY), JSON.stringify({ items: cat, publishedAt: payload.publishedAt }))
    }
