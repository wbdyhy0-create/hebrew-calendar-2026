...
/** תאריך עברי מלא בגימטריא כולל שנה (למשל תצוגת יום בדיספליי). */
export function formatHebrewDateFullGematriya(gregDate: Date): string {
  const hd = new HDate(civilDateToUtcNoon(gregDate));
  return hd.renderGematriya(true, false).trim();
}
...
export function getDayEventsByGregorianDate(
  ...
  const events = HebrewCalendar.calendar({
    ...
    candleLightingMins: typeof opts?.candleLightingMins === 'number' ? opts.candleLightingMins : undefined,
  });
