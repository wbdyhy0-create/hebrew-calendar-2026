...
export async function downloadPdfFromHtml(
  ...
  async function renderWithHtml2CanvasThenPdf() {
    ...
    function findOpaqueBounds(canvas: HTMLCanvasElement): CanvasBounds | null {
      const w = canvas.width;
      const h = canvas.height;
      if (!w || !h) return null;
      const ctx = canvas.getContext('2d', { willReadFrequently: true });
      ...
      const hasOpaqueInCol = (x: number) => {
        for (let y = 0; y < h; y += stride) {
          if (isOpaqueAt(x, y)) return true;
        }
        return false;
      };
      const hasOpaqueInRow = (y: number) => {
      ...
      while (left < right && !hasOpaqueInCol(left)) left++;
      while (right > left && !hasOpaqueInCol(right)) right--;
      while (top < bottom && !hasOpaqueInRow(top)) top++;
    ...
    function cropCanvas(canvas: HTMLCanvasElement, b: CanvasBounds, padPx: number): HTMLCanvasElement {
      const w = canvas.width;
      const h = canvas.height;
      let left = Math.max(0, b.left - padPx);
      let top = Math.max(0, b.top - padPx);
      let right = Math.min(w - 1, b.right + padPx);
      let bottom = Math.min(h - 1, b.bottom + padPx);
